...
private const val MEDIA_METADATA_DURATION_MS_KEY = "media_metadata_duration_ms"
private const val MEDIA_METADATA_THUMBNAIL_URI_KEY = "media_metadata_thumbnail_uri"
...
private fun Bundle.setExtras(
    ...
    thumbnailUri?.let { putString(MEDIA_METADATA_THUMBNAIL_URI_KEY, it) }
}
...
val MediaMetadata.durationMsFromExtras: Long?
    get() = extras?.run {
        getLong(MEDIA_METADATA_DURATION_MS_KEY)
            .takeIf { containsKey(MEDIA_METADATA_DURATION_MS_KEY) }
    }

val MediaMetadata.thumbnailUriFromExtras: String?
    get() = extras?.run {
        getString(MEDIA_METADATA_THUMBNAIL_URI_KEY)
            .takeIf { containsKey(MEDIA_METADATA_THUMBNAIL_URI_KEY) }
    }

fun MediaItem.copy(
    positionMs: Long? = this.mediaMetadata.positionMs,
    durationMs: Long? = this.mediaMetadata.durationMs ?: this.mediaMetadata.durationMsFromExtras,
