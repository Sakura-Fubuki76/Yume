...
import com.sakurafubuki.yume.feature.player.extensions.registerForSuspendActivityResult
...
import com.sakurafubuki.yume.feature.player.service.PlayerService
...
class PlayerActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val serversProvider = { webDavServersById }

            CompositionLocalProvider(LocalUseMaterialYouControls provides (uiState.playerPreferences?.useMaterialYouControls == true)) {
                YumeTheme(darkTheme = true) {
                    MediaPlayerScreen(
                        player = player,
                        viewModel = viewModel,
                        playerPreferences = uiState.playerPreferences ?: return@YumeTheme,
                        webDavServersById = serversProvider,
    ...
    private suspend fun playVideo(uri: Uri) = withContext(Dispatchers.Default) {
        ...
        val thumbnailMap = VideoThumbnailStore.thumbnailUriMap.toMap()
        val durationMap = VideoThumbnailStore.durationMsMap.toMap()
            ...
            MediaItem.Builder().apply {
                ...
                setMediaMetadata(
                    MediaMetadata.Builder().apply {
                        ...
                        thumbUri?.let { setArtworkUri(it.toUri()) }
                        durationMs?.let { setDurationMs(it) }
                    }.build(),
                )
                if (index == mediaItemIndexToPlay) {
    ...
    override fun onDestroy() {
        if (isFinishing && !playInBackground) {
