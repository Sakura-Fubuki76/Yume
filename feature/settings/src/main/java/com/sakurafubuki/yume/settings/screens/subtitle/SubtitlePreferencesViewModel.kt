...
class SubtitlePreferencesViewModel @Inject constructor(
    ...
    fun onEvent(event: SubtitlePreferencesUiEvent) {
        when (event) {
            ...
            is SubtitlePreferencesUiEvent.UpdateCustomFontsDirectory -> updateCustomFontsDirectory(event.value)
        }
    }
    ...
    private fun updateCustomFontsDirectory(value: String) {
        viewModelScope.launch {
            preferencesRepository.updatePlayerPreferences { it.copy(customFontsDirectory = value) }
        }
    }
}
...
sealed interface SubtitlePreferencesUiEvent {
    ...
    data class UpdateCustomFontsDirectory(val value: String) : SubtitlePreferencesUiEvent
}
